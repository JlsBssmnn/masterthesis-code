...
def generate_image(config):
    ...
    noise = noise * intensity[:, np.newaxis, np.newaxis]
    ...
    watershed_result = watershed(distance + noise, markers, watershed_line=True)
    ...
    membrane_mask = cell_image == 0
    noise = perlin.generate_perlin_noise_3d(config.limits, config.noise_resolution_post)
    post_distance_noise = noise * config.noise_intensity_post
    post_distance_noise[post_distance_noise < 0] = 0

    post_distance = skfilters.gaussian(post_distance, sigma=config.gaussian_sigma)
    post_distance += post_distance_noise
    cell_image[post_distance <= config.membrane_threshold] = config.membrane_color
    cell_image[post_distance > config.membrane_threshold] = config.cell_color
    cell_image[membrane_mask] = config.membrane_color
    logging.info('made membranes thicker')

    blob_image = create_blob_image(cell_image, config)
    logging.info('Created blobs')

    return np.stack([cell_image, blob_image], axis=0)
...
def create_blob_image(cell_image, config):
    ...
    for blob in blob_points:
        if cell_image[tuple(blob)] == config.membrane_color:
            continue
        ...
        blob_membrane_voxels = dist_trans[blob_mask & (image_patch == config.membrane_color)].flatten()
