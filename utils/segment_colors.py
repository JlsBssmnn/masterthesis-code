...
# This file is copied from https://github.com/google/neuroglancer/blob/master/python/neuroglancer/segment_colors.py
...
def get_colors(seed):
    ...
    colorvecs = []
    for i in range(1000):
        color = hex_string_from_segment_id(seed, i)
        if color in colors:
            break
        colors.add(color)

        vec = [int(color[1:3], 16) / 255, int(color[3:5], 16) / 255, int(color[5:7], 16) / 255]
        colorvecs.append(vec)

    return np.array(colorvecs)
